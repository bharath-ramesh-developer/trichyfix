...
export class Services implements OnInit {
  ...
  async fetchInitialData() {
    try {
      const catRes = await fetch('http://localhost:3000/api/providers/categories');
      ...
      const areaRes = await fetch('http://localhost:3000/api/providers/areas');
  ...
  async fetchProviders() {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/providers?${params.toString()}`);
